...
class DingReadyMine {
	...
	ddReady = ({
		...
		dd.ready(function() {
			...
			let ddJsApiHandle = {
				...
				// 选取部门
				departments: () => {
					self.departmentsFn({context: context})();
				}
			}
						...
						if (messageBoard.indexOf(el.name) != -1) {
				            for (let ele of messageBoard) {
				                if(otherData.writeMsg.indexOf(ele) != -1) {
				                	common.dispatchFn({
				                		context: context,
				                		val: {writeMsg: otherData.writeMsg.substring(0,otherData.writeMsg.length - 1)}
				                	});
	...
	*/
	uploadFileFn = ({
		...
		dd.biz.util.uploadAttachment({
			image:{multiple:true,compress:false,max:9,spaceId: otherData.spaceId},
		    ...
		    onSuccess : function(result) {
		    	...
		    	dd.device.notification.alert({
					message: '上传文件时接收到的spaceid--- ' + JSON.stringify(otherData),
					title: "选取文件",
					buttonName: "确定"
				});
				dd.device.notification.alert({
		    	...
		    	if (result.data.length) {
		    		if (!otherData.isBZUpload) {
		    			enclosure = context.state.enclosure.concat(result.data);
			    		common.dispatchFn({
							context: context,
							val: {
								enclosure: enclosure
	...
	*/
	previewFileFn = ({
		...
		/*dd.device.notification.alert({
			message: "预览previewFileFn文件信息---" + JSON.stringify(otherData),
			title: "选取文件",
			buttonName: "确定"
		});*/
